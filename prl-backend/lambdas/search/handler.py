...
class LambdaHandler:
    ...
    def __init__(self, settings_name="zappa_settings", session=None):
        ...
        if not self.settings:
            ...
            # https://github.com/Miserlou/Zappa/issues/1164
                ...
                # The app module - wrap in try-except to prevent broken singleton caching
                try:
                    ...
                    logger.error(
                        f"Failed to import app module {self.settings.APP_MODULE}: {import_error}"
                    )
                    # Clear the singleton so next request will retry
                    LambdaHandler.__instance = None
                    self.settings = None
                    raise RuntimeError(
                        f"Failed to import app module: {import_error}. "
                        "Singleton cleared - next request will retry initialization."
                    ) from import_error
            ...
            # Validate that we have a valid WSGI app before caching
            if wsgi_app_function is None:
                app_module = getattr(self.settings, "APP_MODULE", "unknown")
                app_function = getattr(self.settings, "APP_FUNCTION", "unknown")
                error_msg = (
                    f"Failed to load WSGI app: {app_module}.{app_function} is None. "
                    "This usually indicates the app module failed to initialize properly."
                )
                logger.error(error_msg)
                # Clear the singleton so next request will retry initialization
                LambdaHandler.__instance = None
                self.settings = None
            ...
            # Verify the wrapped app is callable
            if not callable(self.wsgi_app):
                logger.error("WSGI app is not callable after wrapping")
                LambdaHandler.__instance = None
                self.settings = None
                raise RuntimeError("WSGI app is not callable - initialization failed")
    ...
    def _process_response_body(
        ...
        if settings.BINARY_SUPPORT:
            ...
            additional_text_mimetypes = getattr(
                settings, "ADDITIONAL_TEXT_MIMETYPES", None
            )
            ...
            if response.headers.get(
                "Content-Encoding"
            ):  # Assume br/gzip/deflate/etc encoding
        ...
        # This is an HTTP-protocol API Gateway event or Lambda url event with payload format version 2.0
            ...
            try:
                ...
                with Response.from_app(self.wsgi_app, environ) as response:
                    ...
                    if response.data:
                        response_body, response_is_base_64_encoded = (
                            self._process_response_body(response, settings=settings)
                        )
            ...
            if event.get("httpMethod", None):
                ...
                with Response.from_app(self.wsgi_app, environ) as response:
                    ...
                    if "multiValueHeaders" in event:
                        ...
                        for key, value in response.headers:
                            zappa_returndict["multiValueHeaders"][key] = (
