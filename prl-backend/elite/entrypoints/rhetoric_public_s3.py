"""Entry point for rhetoric public data export to S3 (daily)."""
...
with job_collector("rhetoric-public-s3") as c:
    # Collect data
    print("COLLECTING DATA; SAVING LOCALLY")
    subprocess.run(
    ...
    # Push to S3 if .tmp directory exists (collect.py creates it only if there are changes)
    tmp_dir = os.path.join(public_dir, ".tmp")
    if os.path.isdir(tmp_dir):
        print("PUSHING DATA; SENDING TO S3")
        import boto3
        import shutil
